...
# Tom's implemetation of EMR
def EMR(y_true,y_pred):
    
    hl_num=(np.sum(np.logical_not(np.logical_xor(y_true,y_pred)), axis=1) == 13).sum()
    
    return float(hl_num)/y_true.shape[0]
...
def example_based_recall(y_true,y_pred):
    ...
    p_n=np.array(recall_num).astype(np.float32)
    p_d=np.array(recall_den).astype(np.float32)
